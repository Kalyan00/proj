...
begin
  StringGrid1.Cells[1,0]:='Тест 1';
  StringGrid1.Cells[0,1]:='ФИО 1';
  StringGrid1.Cells[2,0]:='Тест 2';
  StringGrid1.Cells[0,2]:='ФИО 2';
  x:=1000;
  y:=1000;
end;
...
begin
  ...
  for i:=oldr to y+1+y___ do
    for j:=0 to x+1+x___ do
      if j=0 then
        if i<y+1 then
          StringGrid1.Cells[j,i]:='ФИО '+IntToStr(j) else else
        StringGrid1.Cells[j,i]:='';
  for j:=oldc to x+1+x___ do
    for i:=0 to y+1+y___ do
      if i=0 then
        if j<x+1 then
          StringGrid1.Cells[j,i]:='Тест '+IntToStr(j) else else
        StringGrid1.Cells[j,i]:='';
